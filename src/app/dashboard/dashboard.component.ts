...
export class DashboardComponent implements OnInit {
  ...
  printCurrentMonthGrid(dt) {
     ...
     * Get Collection data
     */
    let docdata = this._db
      .collection(this.userdet.email)
      .valueChanges()
      .subscribe(Response => {
        ...
        for (let i in Response) {
          if (
            Response[i]["intime"] > tmpdate.getTime() &&
            Response[i]["outtime"] < tmpenddt.getTime()
          ) {
        ...
        let cnt = new Date(tmpdt).getDay() + 1;
        let dtarr = [];
        for (let i = mntstartdt; i <= mntenddt; i++) {
          ...
          for (let k in firearr) {
            ...
            if (intm.getDate() == i) {
              let inhr =
                intm.getHours() < 10 ? "0" + intm.getHours() : intm.getHours();
              let inmin =
                intm.getMinutes() < 10
                  ? "0" + intm.getMinutes()
                  : intm.getMinutes();
  ...
  saveTimesheet() {
    ...
    if (this.outhours && this.outmins) {
      ...
      if (outdt.getTime() <= indt.getTime()) {
        alert(
          "Your OUT time cannot be greater/equal to IN time, kindly rectify"
        );
        return;
      } else {
  ...
  setPreviousMonth() {
    ...
    if (
      dt.getMonth() == todaydt.getMonth() &&
      dt.getFullYear() == todaydt.getFullYear()
    ) {
